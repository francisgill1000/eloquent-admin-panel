...
const AddDeal = ({ options, onSuccess = (e) => e }) => {
    ...
    useEffect(() => {
        if (open) {
            setForm({ ...defaultPayload, ...options.item });
        }
    }, [open, options.leadId]);
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post(options.endpoint, form);

            onSuccess({
                title: `${options.page_title} Saved`,
                description: `${options.page_title} created successfully`,
                data: response.data,
            });
    ...
    return (
        <>
            ...
            >
                <Handshake size={20} /> Convert to Deal
            </button>
                ...
                <DialogContent className="max-w-lg bg-primary text-muted">
                    <DialogHeader>
                        ...
                        <p className="text-sm text-muted-foreground italic mb-3">
                            Let's create a new {options.page_title} — just speak or type the details.
                        </p>
                    </DialogHeader>
